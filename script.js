...
// Kivicube AR 发布链接。把每一关对应的 Kivicube 链接填到这里。
const lesson1KivicubeUrl = "https://www.kivicube.com/face-scenes/lVfIzU5RK7NLCJsfNiRCb83KEPWP5GbW";
const lesson2KivicubeUrl = "https://www.kivicube.com/face-scenes/lVfIzU5RK7NLCJsfNiRCb83KEPWP5GbW";
const lesson3KivicubeUrl = "https://www.kivicube.com/face-scenes/lVfIzU5RK7NLCJsfNiRCb83KEPWP5GbW";
const lesson4KivicubeUrl = "https://www.kivicube.com/face-scenes/lVfIzU5RK7NLCJsfNiRCb83KEPWP5GbW";
const lesson5KivicubeUrl = "https://www.kivicube.com/face-scenes/lVfIzU5RK7NLCJsfNiRCb83KEPWP5GbW";
...
const ui = {
  ...
  arButton: document.querySelector("#ar-btn"),
  arOverlay: document.querySelector("#ar-overlay"),
  arFrame: document.querySelector("#kivicube-frame"),
  arOpenLink: document.querySelector("#ar-open-link")
};
...
function renderLesson() {
  ...
  setKivicubeEntry(currentLesson);
...
function getKivicubeUrl(index) {
  return ASSETS.kivicube[index].trim();
}

function setKivicubeEntry(index) {
  const arUrl = getKivicubeUrl(index);
  ui.arTitle.textContent = `${lessons[index].level} AR 观察`;
  if (arUrl) {
    ui.arButton.disabled = false;
    ui.arButton.classList.remove("disabled");
    ui.arStatus.textContent = "点击后在页面内打开 Kivicube";
    return;
  }

  ui.arButton.disabled = true;
  ui.arButton.classList.add("disabled");
  ui.arStatus.textContent = "Kivicube 链接待替换";
}

function openKivicube() {
  const arUrl = getKivicubeUrl(currentLesson);
  if (!arUrl) {
    ui.arStatus.textContent = "请先在 script.js 顶部填入 Kivicube 链接";
    return;
  }

  ui.arFrame.src = arUrl;
  ui.arOpenLink.href = arUrl;
  ui.arOverlay.hidden = false;
}

function closeKivicube() {
  ui.arOverlay.hidden = true;
  ui.arFrame.removeAttribute("src");
}
...
ui.arButton.addEventListener("click", openKivicube);
document.querySelector("#close-ar").addEventListener("click", closeKivicube);
